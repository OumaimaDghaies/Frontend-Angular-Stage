...
export class EmployeeComponent implements OnInit, AfterViewInit{
  ...
  constructor(private manageEmployeeService: ManageEmployeeService,private messageService: MessageService) { }
  ...
  saveEmployee(){
    this.manageEmployeeService.saveEmployee(this.employee,this.newEmployee)
    .subscribe({next: (data) => {  
      this.EmployeeChange.emit(this.employee)
      this.saveEChange.emit(true);
      this.messageService.add({ severity: 'success', summary: 'Succès', detail: 'Employee enregistré avec succès.' });
